describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(let i = 0; i < prodItemsData.length; i++){
      let productVal = prodItemsData[i];
      console.log(`Checking product item ${i + 1}/${prodItemsData.length}`);

      if (productVal.title.length == 0) { allArePopulated = false; }
      if (productVal.price.length == 0) { allArePopulated = false; }
      if (productVal.image.length == 0) { allArePopulated = false; }
    }
    expect(allArePopulated).toBe(true);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let buttonCart = await shadowRoot.$('button');

    await buttonCart.click();

    let buttonTextProperty = await buttonCart.getProperty('innerText');
    let buttonTextValue = await buttonTextProperty.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartCountProp = await cartCountHandle.getProperty('innerText');
    let cartCount = await cartCountProp.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const buttonCart = await shadowRoot.$('button');

      const buttonTextProperty = await buttonCart.getProperty('innerText');
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    console.log('Checking localStorage to see if cart values are correct');
